...
bool PointClouds::load(
    const std::string& folder_with_point_clouds,
    const std::string& poses_file_name,
    bool decimation,
    double bucket_x,
    double bucket_y,
    double bucket_z)
    ...
    double bucket_z)
{
    ...
    std::cout << "sum_points before/after decimation: " << sum_points_before_decimation << " / " << sum_points_after_decimation
              << std::endl;
    print_point_cloud_dimension();
    return true;
}
...
bool PointClouds::save_poses(const std::string file_name, bool is_subsession)
{
    ...
    if (!outfile.good())
    {
        ...
        std::cout << "if you can see only '' it means there is no filename assigned to poses, please read manual or ask for support"
                  << std::endl;
    ...
    for (size_t i = 0; i < this->point_clouds.size(); i++)
    {
        if (!is_subsession)
        {
            ...
            outfile << this->point_clouds[i].m_pose(0, 0) << " " << this->point_clouds[i].m_pose(0, 1) << " "
                    << this->point_clouds[i].m_pose(0, 2) << " " << this->point_clouds[i].m_pose(0, 3) << std::endl;
            outfile << this->point_clouds[i].m_pose(1, 0) << " " << this->point_clouds[i].m_pose(1, 1) << " "
                    << this->point_clouds[i].m_pose(1, 2) << " " << this->point_clouds[i].m_pose(1, 3) << std::endl;
            outfile << this->point_clouds[i].m_pose(2, 0) << " " << this->point_clouds[i].m_pose(2, 1) << " "
                    << this->point_clouds[i].m_pose(2, 2) << " " << this->point_clouds[i].m_pose(2, 3) << std::endl;
...
double get_mean_uncertainty_xyz_impact6x6(
    std::vector<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>& uncertainty_before,
    std::vector<Eigen::Matrix<double, 6, 6, Eigen::RowMajor>>& uncertainty_after)
{
    ...
    {
        sum_before +=
            ((uncertainty_before[i].coeffRef(0, 0) + uncertainty_before[i].coeffRef(1, 1) + uncertainty_before[i].coeffRef(2, 2)) / 3.0);
        sum_after +=
            ((uncertainty_after[i].coeffRef(0, 0) + uncertainty_after[i].coeffRef(1, 1) + uncertainty_after[i].coeffRef(2, 2)) / 3.0);
    }
...
double get_mean_uncertainty_xyz_impact7x7(
    std::vector<Eigen::Matrix<double, 7, 7, Eigen::RowMajor>>& uncertainty_before,
...
double get_mean_uncertainty_xyz_impact(
    std::vector<Eigen::SparseMatrix<double>>& uncertainty_before, std::vector<Eigen::SparseMatrix<double>>& uncertainty_after)
{
    double mui = 0.0;
...
bool PointClouds::load_whu_tls(
    std::vector<std::string> input_file_names, bool is_decimate, double bucket_x, double bucket_y, double bucket_z, bool calculate_offset)
{
...
{
    PointCloudDimensions dim{ std::numeric_limits<double>::max(),
...
void PointClouds::print_point_cloud_dimension()
{
    ...
    std::cout << "X (length): " << std::fixed << std::setprecision(3) << dims.x_min << " / " << std::fixed << std::setprecision(3)
              << dims.x_max << " / " << std::fixed << std::setprecision(3) << dims.length << std::endl;
    std::cout << "Y (width) : " << std::fixed << std::setprecision(3) << dims.y_min << " / " << std::fixed << std::setprecision(3)
              << dims.y_max << " / " << std::fixed << std::setprecision(3) << dims.width << std::endl;
    std::cout << "Z (height): " << std::setprecision(3) << dims.z_min << " / " << std::fixed << std::setprecision(3) << dims.z_max << " / "
              << std::fixed << std::setprecision(3) << dims.height << std::endl;

    std::cout << "------------------------" << std::endl;
}

bool PointClouds::load_3DTK_tls(
    std::vector<std::string> input_file_names, bool is_decimate, double bucket_x, double bucket_y, double bucket_z)
{
    ...
    {
        std::cout << "loading file " << i + 1 << "/" << input_file_names.size() << " ("
                  << (std::filesystem::path(input_file_names[i]).filename().string()) << ")" << std::endl;
        PointCloud pc;
        pc.m_pose = Eigen::Affine3d::Identity();
        pc.m_initial_pose = pc.m_pose;
        pc.pose = pose_tait_bryan_from_affine_matrix(pc.m_pose);
