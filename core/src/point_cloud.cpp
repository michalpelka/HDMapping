...
bool PointCloud::load(const std::string& file_name)
{
    ...
    // Listing PLY content
    {
        ...
        for (const auto& element : data)
        {
            ...
            {
                std::cout << "    - " << prop.key << " -- type: " << (prop.data->isList ? "list of " : "") << prop.data->type.name()
...
void PointCloud::reindex(std::vector<PointBucketIndexPair>& ip, std::vector<Eigen::Vector3d>& points, GridParameters params)
{
    ...
    // std::sort(index_pairs.begin(), index_pairs.end(), [](const PointBucketIndexPair& a, const PointBucketIndexPair& b) { return
    // ((a.index_of_bucket == b.index_of_bucket) ? (a.index_pose < b.index_pose) : (a.index_of_bucket < b.index_of_bucket)); });
    std::sort(
        ip.begin(),
        ip.end(),
...
{
    std::cout << "Number_of_buckets X,Y,Z: " << rgd_params.number_of_buckets_X << ", " << rgd_params.number_of_buckets_Y << ", "
              << rgd_params.number_of_buckets_Z << std::endl;
    std::cout << "bounding box: x(" << rgd_params.bounding_box_min_X << "," << rgd_params.bounding_box_max_X << "), y("
              << rgd_params.bounding_box_min_Y << "," << rgd_params.bounding_box_max_Y << ") , z(" << rgd_params.bounding_box_min_Z << ","
              << rgd_params.bounding_box_max_Z << ")" << std::endl;
}
    ...
    double search_radious)
{
    ...
    for (size_t ii = job->index_begin_inclusive; ii < job->index_end_exclusive; ii++)
    {
        ...
        if (p.x() < rgd_params.bounding_box_min_X || p.x() > rgd_params.bounding_box_max_X)
            continue;
        if (p.y() < rgd_params.bounding_box_min_Y || p.y() > rgd_params.bounding_box_max_Y)
            continue;
        ...
        if (index_bucket >= 0 && index_bucket < rgd_params.number_of_buckets)
        {
            ...
            for (int i = sx; i < stx; i++)
            {
                for (int j = sy; j < sty; j++)
                {
                    ...
                    {
                        index_next_bucket = index_bucket + i * rgd_params.number_of_buckets_Y * rgd_params.number_of_buckets_Z +
...
{
    double sq3d2 = 0.86602540378443864676, c2d3 = c[2] / 3, c2sq = c[2] * c[2], Q = (3 * c[1] - c2sq) / 9,
...
inline void gpuldu3(double* A, int* P)
{
    ...
    P[0] = abs(A[3 * 1 + 0]) > abs(A[3 * 0 + 0]) ? (abs(A[3 * 2 + 0]) > abs(A[3 * 1 + 0]) ? 2 : 1)
    ...
    A[3 * P[2] + 2] =
        A[3 * P[2] + 2] - A[3 * P[0] + 2] * A[3 * P[2] + 0] * A[3 * P[0] + 0] - A[3 * P[1] + 2] * A[3 * P[2] + 1] * A[3 * P[1] + 1];
}
...
inline void gpuSVD(double* _A, double* U, double* _S, double* V)
{
    ...
    S[1] =
        AA[0][0] * AA[1][1] + AA[2][2] * AA[0][0] + AA[2][2] * AA[1][1] - AA[2][1] * AA[1][2] - AA[2][0] * AA[0][2] - AA[1][0] * AA[0][1];
    ...
    y[0] = y[1] = y[2] = 0;
    k = abs(LDU[P[1]][1]) < abs(LDU[P[0]][0]) ? (abs(LDU[P[2]][2]) < abs(LDU[P[1]][1]) ? 2 : 1)
    ...
    /* Pick the component of D nearest to 0 */
    y[0] = y[1] = y[2] = 0;
    k = abs(LDU[P[0]][0]) < abs(LDU[P[2]][2]) ? (abs(LDU[P[0]][0]) < abs(LDU[P[1]][1]) ? 0 : 1)
    ...
    case 1:
         ...
         */
        y[0] = y[1] = y[2] = 0;
        k = abs(U[3 * 0 + 0]) < abs(U[3 * 0 + 2]) ? (abs(U[3 * 0 + 0]) < abs(U[3 * 0 + 1]) ? 0 : 1)
    ...
    double search_radious)
{
    for (size_t ii = job->index_begin_inclusive; ii < job->index_end_exclusive; ii++)
    {
        ...
        if (point.x() < rgd_params.bounding_box_min_X || point.x() > rgd_params.bounding_box_max_X)
            continue;
        if (point.y() < rgd_params.bounding_box_min_Y || point.y() > rgd_params.bounding_box_max_Y)
            continue;
        if (point.z() < rgd_params.bounding_box_min_Z || point.z() > rgd_params.bounding_box_max_Z)
            continue;
...
void PointCloud::compute_normal_vectors(double search_radious)
{
    ...
    std::cout << "number_points_horizontal: " << number_points_horizontal << " number_points_vertical: " << number_points_vertical
              << std::endl;
}
...
void PointCloud::decimate(double bucket_x, double bucket_y, double bucket_z)
{
    ...
    std::vector<Eigen::Vector3d> n_colors;
    ...
    for (int i = 1; i < ip.size(); i++)
    {
        if (ip[i - 1].index_of_bucket != ip[i].index_of_bucket)
        {
            ...
            if (points_type.size() == points_local.size())
                n_points_type.emplace_back(points_type[ip[i].index_of_point]);
            if (intensities.size() == points_local.size())
                n_intensities.emplace_back(intensities[ip[i].index_of_point]);
            if (timestamps.size() == points_local.size())
                n_timestamps.emplace_back(timestamps[ip[i].index_of_point]);
            if (colors.size() == points_local.size())
                n_colors.emplace_back(colors[ip[i].index_of_point]);
        }
    }

    std::cout << "colors.size(): " << colors.size() << std::endl;
    std::cout << "n_colors.size(): " << n_colors.size() << std::endl;
    ...
    double intersection_width)
{
    ...
    if (this->visible)
    {
        ...
        for (int i = 0; i < this->points_local.size(); i += viewer_decimate_point_cloud)
        {
            ...
            if (this->show_color)
            {
                if (this->colors.size() == this->points_local.size())
                {
                    glColor3f(this->colors[i].x(), this->colors[i].y(), this->colors[i].z());
                }
            }
            //
