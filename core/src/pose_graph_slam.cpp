...
bool PoseGraphSLAM::optimize(PointClouds& point_clouds_container)
{
    ...
    if (is_tait_bryan_angles)
    {
        ...
        for (int iter = 0; iter < iterations; iter++)
        {
            ...
            Eigen::SparseMatrix<double> AtPA(
                point_clouds_container.point_clouds.size() * 6, point_clouds_container.point_clouds.size() * 6);
            ...
            for (size_t i = 0; i < edges.size(); i++)
            {
                ...
                if (is_wc)
                {
                    ...
                    relative_pose_obs_eq_rodrigues_wc(
                        ...
                        relative_pose.sy,
                        relative_pose.sz);
                    relative_pose_obs_eq_rodrigues_wc_jacobian(
                        jacobian,
                        poses[edges[i].index_from].px,
                        poses[edges[i].index_from].py,
                        poses[edges[i].index_from].pz,
    ...
    if (is_quaternion)
    {
        ...
        for (int iter = 0; iter < iterations; iter++)
        {
            ...
            for (size_t i = 0; i < edges.size(); i++)
            {
                ...
                if (is_wc)
                {
                    relative_pose_obs_eq_quaternion_wc(
                        ...
                        relative_pose.q2,
                        relative_pose.q3);
                    relative_pose_obs_eq_quaternion_wc_jacobian(
                        jacobian,
                        poses[edges[i].index_from].px,
                        poses[edges[i].index_from].py,
                        poses[edges[i].index_from].pz,
                ...
                else if (is_cw)
                {
                    relative_pose_obs_eq_quaternion_cw(
                        ...
                        relative_pose.q2,
                        relative_pose.q3);
                    relative_pose_obs_eq_quaternion_cw_jacobian(
                        jacobian,
                        poses[edges[i].index_from].px,
                        poses[edges[i].index_from].py,
                        poses[edges[i].index_from].pz,
            ...
            Eigen::SparseMatrix<double> AtPA(
                point_clouds_container.point_clouds.size() * 7, point_clouds_container.point_clouds.size() * 7);
...
std::vector<Eigen::SparseMatrix<double>> PoseGraphSLAM::compute_covariance_matrices_and_rms(
    std::vector<PointCloud>& point_clouds, double& rms)
{
...
void PoseGraphSLAM::calculate_edges(std::vector<PointCloud>& point_clouds)
{
    if (pair_wise_matching_type == PairWiseMatchingType::general)
    {
        for (size_t i = 0; i < edges.size(); i++)
        {
            std::cout << "PROGRESS calculating edges: " << i + 1 << " of " << edges.size() << "[" << edges[i].index_from << ","
                      << edges[i].index_to << "]" << std::endl;
    ...
    else if (pair_wise_matching_type == PairWiseMatchingType::pcl_ndt)
    {
        ...
        for (size_t i = 0; i < edges.size(); i++)
        {
            ...
            {
                Eigen::Vector3d p(
                    point_clouds[edges[i].index_to].points_local[j].x(),
            ...
            {
                Eigen::Vector3d p(
                    point_clouds[edges[i].index_from].points_local[j].x(),
            ...
            std::cout << "Normal Distributions Transform has converged:" << ndt.hasConverged() << " score: " << ndt.getFitnessScore()
                      << std::endl;
    ...
    else if (pair_wise_matching_type == PairWiseMatchingType::pcl_icp)
    {
        ...
        for (size_t i = 0; i < edges.size(); i++)
        {
            ...
            for (size_t j = 0; j < point_clouds[edges[i].index_to].points_local.size(); j++)
            {
                ...
                Eigen::Vector3d nv(
                    point_clouds[edges[i].index_to].normal_vectors_local[j].x(),
            ...
            for (size_t j = 0; j < point_clouds[edges[i].index_from].points_local.size(); j++)
            {
                ...
                pcl::PointXYZRGBNormal p_pcl;
                Eigen::Vector3d nv(
                    point_clouds[edges[i].index_from].normal_vectors_local[j].x(),
...
bool PoseGraphSLAM::optimize_with_GTSAM(PointClouds& point_clouds_container)
{
    ...
    for (size_t i = 0; i < edges.size(); i++)
    {
        auto odometryNoise = noiseModel::Diagonal::Variances((Vector(6) << 1, 1, 1, 1, 1, 1).finished());
