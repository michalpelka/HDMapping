...
void reindex_job(
    int i, NDT::Job* job, std::vector<Point3D>* points, std::vector<NDT::PointBucketIndexPair>* pairs, NDT::GridParameters rgd_params)
{
...
void reindex_jobi(
    int i, NDT::Job* job, std::vector<Point3Di>* points, std::vector<NDT::PointBucketIndexPair>* pairs, NDT::GridParameters rgd_params)
{
    for (size_t ii = job->index_begin_inclusive; ii < job->index_end_exclusive; ii++)
    {
    ...
    std::vector<Point3D>& points, std::vector<NDT::PointBucketIndexPair>& index_pair, NDT::GridParameters& rgd_params, int num_threads)
{
    ...
    std::sort(
        index_pair.begin(),
        index_pair.end(),
        [](const NDT::PointBucketIndexPair& a, const NDT::PointBucketIndexPair& b)
        {
...
void NDT::reindex(
    std::vector<Point3Di>& points, std::vector<NDT::PointBucketIndexPair>& index_pair, NDT::GridParameters& rgd_params, int num_threads)
{
    index_pair.resize(points.size());
...
std::vector<Eigen::Vector3d> get_points_normal_distribution(
    const Eigen::Matrix3d& covar, const Eigen::Vector3d& mean, const int num_points = 100)
{
...
bool NDT::optimize(std::vector<PointCloud>& point_clouds, bool compute_only_mahalanobis_distance, bool compute_mean_and_cov_for_bucket)
{
    ...
    for (int iter = 0; iter < number_of_iterations; iter++)
    {
        ...
        for (int bi = 0; bi < buckets_external.size(); bi++)
        {
            ...
            {
                AtPAtmp[i] =
                    Eigen::SparseMatrix<double>(point_clouds.size() * number_of_unknowns, point_clouds.size() * number_of_unknowns);
