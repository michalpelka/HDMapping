    ...
    if (worker_data.size() != 0)
    {
        ...
        for (int i = 0; i < worker_data.size(); i++)
        {
                ...
                for (int ii = 0; ii < intermediate_points.size(); ii++)
                {
                    ...
                    if (r_l > 0.5 && intermediate_points[ii].index_pose != -1 && r_l < params.max_distance_lidar_rigid_sf)
                    {
                        double polar_angle_deg_l =
                            atan2(intermediate_points[ii].point.y(), intermediate_points[ii].point.x()) / M_PI * 180.0;
            ...
            else
            {
                ...
                update_rgd(
                    params.in_out_params_outdoor, params.buckets_outdoor, pg, worker_data[i].intermediate_trajectory[0].translation());
            }
