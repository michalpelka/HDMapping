    ...
    CommonData& common_data)
{
    ...
    for (size_t i = 0; i < trajectories.size(); i++)
    {
        ...
        if (trajectories[i].line_width < 1)
            trajectories[i].line_width = 1;
...
bool ProjectSettings::save(
    ...
    std::vector<LAZSector>& sectors,
    const std::vector<ROIwithConstraints>& rois_with_constraints,
    const CommonData& common_data)
{
    ...
    for (const auto& t : trajectories)
    {
        ...
        nlohmann::json trajectory{ { "trajectory_file", t.trajectory_file },
                                   { "trajectory_motion_model_file", t.motion_model_file_name },
                                   { "colors", { t.color_x, t.color_y, t.color_z } } };
    ...
    for (const auto& rwc : rois_with_constraints)
    {
        ...
        {
            nlohmann::json json_roi{ { "trajectory_name", c.trajectory_file_name },
    ...
    CommonData& common_data)
{
    ...
    {
        add_trajectory(
            t_json["trajectory_file"],
            t_json["trajectory_motion_model_file"],
            t_json["colors"][0],
            t_json["colors"][1],
            t_json["colors"][2]);
    }
    ...
    for (size_t i = 0; i < trajectories.size(); i++)
    {
        ...
        {
            total_length +=
                (trajectories[i].fused_trajectory[j].m_pose.translation() - trajectories[i].fused_trajectory[j - 1].m_pose.translation())
                    .norm();
        }
    }
...
bool ProjectSettings::add_trajectory(
    const std::string& file_name, const std::string& file_motion_model_name, float color_x, float color_y, float color_z)
{
            ...
            for (const auto& trj : trajectories)
            {
                if (c.trajectory_file_name == trj.trajectory_file)
                {
                    auto it = std::lower_bound(
                        trj.fused_trajectory.begin(),
                        trj.fused_trajectory.end(),
                        c.time_stamp,
                    ...
                    if (fabs(trj.fused_trajectory[index].timestamp - c.time_stamp) < 0.1)
                    {
                        ...
                        glBegin(GL_LINES);
                        glVertex3f(
                            trj.fused_trajectory[index].m_pose.translation().x(),
...
void ProjectSettings::fuse_with_georeference(
    std::vector<BetweenNode>& between_nodes,
    const std::vector<Eigen::Affine3d>& constraints,
    std::vector<Node>& fused_trajectory,
    const std::vector<Node>& motion_model)
{
    ...
    for (size_t i = 0; i < between_nodes.size(); i++)
    {
        ...
        poses_desired.push_back(
            pose_tait_bryan_from_affine_matrix(motion_model[between_nodes[i].node_outer.index_to_lidar_odometry_odo].m_pose));
    }
    ...
    for (int i = 0; i < between_nodes.size(); i++)
    {
        if (between_nodes[i].node_outer.index_to_gnss != -1)
        {
            ...
            Eigen::Vector3d p_s(
                ...
                0,
                0);
            Eigen::Vector3d p_t(m_geo.translation().x(), m_geo.translation().y(), m_geo.translation().z());
